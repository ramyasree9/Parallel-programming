...
/**************************Main class**********************************/
public class Mat_vect_csr extends Configured implements Tool
{
	/*******************Mapper class*************************/
		...
		public void setup(Context context) throws IOException,InterruptedException
		{
			...
			try{
				while((line=br.readLine())!= null){
					...
					vector.put(Long.parseLong(input[0]),Double.parseDouble(input[1]));
				}
			}
			finally{
				br.close();
	...
	public int run(String args[]) throws Exception
	{
		...
		/* creates a new job with given job name */
    	 	Job job = Job.getInstance(conf, "Mat_vect_csr");	
  		job.setJarByClass(Mat_vect_csr.class);	
	...
	public static void main(String args[]) throws Exception
	{
		...
		int res	=	ToolRunner.run(new Configuration(), new Mat_vect_csr(),args);
		System.exit(res);
